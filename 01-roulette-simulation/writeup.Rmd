...
In this part, we calculate the possibilities of we walk-out with money more than 200, which shows 0.5147.
...
In this part, we calculate the expected winning and it is -45.48, which means if I use the strategy, we all going to lose 45.48 as an expected value. **ADD** Althogh we have over 50% chance to win, we still have a expected value to lose money. That is we only have two choice: win 100 or lose 200. It is so easy to lose more money than win by the strategy.
...
```
In this part, we calculate the average number of plays. From the chart above, we can obviously conclude we nearly never reach the max number of plays. So what's the average? it show 252 far less than 1000.
...
mean(walk_out_money > 200)
```
**ADD**The estimated probability of walking out with extra cash for the parameters B = 200, W = 400, L = 1000, M = 100 is 30.13%. This probability is much lower than before which means we are less likely to be profitable in this scenario comparing with W=300.

```{r}
mean(walk_out_money - 200)
```
**ADD**In this part, we calculate the expected winning and it is -79.84, which means double as W = 300. Althogh we have 30% chance to win 200 dollars, we still have 70% chance to lose all. There is not easy for us to earn from this situation.

```{r}
length(one_series(B = 200, W = 400, L = 1000, M = 100))
```
**ADD**In this part, we calculate the average number of plays. From the chart above, we can obviously conclude we nearly never reach the max number of plays. So what's the average? it show 419 far less than 1000. However, still more than W=300. That is caused by we need a longer road to take the money out.

 Now we loose the limited of winning, which is we would quit when we earn 200 dollars. In this situation, we are greedy and want to earn more, but the reality is that we have lower chance (about 30%) to win out. For most time, we lose to 0. As for the mean earning, it is -79.84 this time. It also revealed. Under this strategy, the more greedy the easier it is to lose
...
**ADD**In this part, it generate a graph showing the walk-out-money. From the graph, most of the money is 0 or 200-300, which means, in most time we quit the game with 0 or win some, and the chance is about half half. Comparing the situation before, which is half half, it is quit similar, which means has near chance to win overall. (figure is 58% to 51%)

```{r}
mean(walk_out_money > 200)
```
**ADD**The estimated probability of walking out with extra cash for the parameters B = 200, W = 300, L = 200, M = 100 is 58.27%. This probability is close as before L = 1000, but a little bit higher, which meand when we limit L,we have more chance to win. And for the expected value, we lose less.

```{r}
mean(walk_out_money - 200)
```

**ADD**In this part, we calculate the expected winning and it is -30.03, which larger than L = 1000.Thus, it means when we gamble much, we lose much. No gambling is earning money from expected value!

```{r}
length(one_series(B = 200, W = 300, L = 200, M = 100))
```
**ADD**When we limit the number of plays to 200, it will drop to 118. That is we cut 80% of the upper limit but only reduce 50% average number of plays. 
...
mean(walk_out_money > 200)
```
**ADD**The estimated probability of walking out with extra cash for the parameters B = 200, W = 300, L = 1000, M = 300 is 52.4%. This probability is similar as before M = 100. It shows the Wager does not limit too much. We can speculate that when we reach the wager to 100, it is far from we lose all.
...
**ADD**In this part, we calculate the expected winning and it is -42.8. To my surprice, it is a little less than M=100. I think they should look similar, and this time is caused by random error.
...
**ADD**In this part, we calculate the average number of plays, which is 198. It is less than we put M = 100. That is because if we lose in a row, we may lose our money quickly because now we can lose 300 dollars for one time, but original version is just 100 dollars.
