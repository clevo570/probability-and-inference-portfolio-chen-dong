...
**ADD**The picture above shows the trend of a player winning money. It can be clearly seen that when winning money, the graph tends to a straight line of 45 degrees. But when you lose money, you will lose steeply. The gambler use near 1000 times to win 100 dollars.


![](/Users/clevo570/Downloads/000038.png)

**ADD**The picture above shows the trend of a player losing money. when the gambler lose money, it looks like a straight line. The gambler use just near 350 times to lose 200  dollars. That is because, when we win we can only win 1 dollar each time. But when we lose it in a row, the money lost every time is doubled.
...
In this part, we calculate the possibilities of we walk-out with money more than 200, which shows 0.504.
...
In this part, we calculate the expected winning and it is -48.9, which means if I use the strategy, we all going to lose 45.85 as an expected value. **ADD** Althogh we have over 50% chance to win, we still have a expected value to lose money. That is we only have two choice: win 100 or lose 200. It is so easy to lose more money than win by the strategy.
...
```
* original: it calculates one person's average number of play
```{r}
length(one_series(B = 200, W = 300, L = 1000, M = 100))
```
In this part, we calculate the average number of plays. From the chart above, we can obviously conclude we nearly never reach the max number of plays. So what's the average? it show 203.77 far less than 1000.
...
number_of_plays[j] <- one_series(B = 200, W = 400, L = 1000, M = 100) %>% length
}
```
...
hist(walk_out_money, breaks = 100)
```
**ADD**In this part, it generate a graph showing the walk-out-money. From the graph, all of the money is 0 or 400, which means, we cannot reach the max number of plays, we will quit the game either lose everything or reach our goal, and the chance is about 70%-30%. Comparing the situation before, which is half half, this time we have less chance to quit while we win.


```{r}
mean(walk_out_money > 200)
```
**ADD**The estimated probability of walking out with extra cash for the parameters B = 200, W = 400, L = 1000, M = 100 is 31.47%. This probability is much lower than before which means we are less likely to be profitable in this scenario comparing with W=300.

```{r}
mean(walk_out_money - 200)
```
**ADD**In this part, we calculate the expected winning and it is -75.01, which means double as W = 300. Althogh we have 30% chance to win 200 dollars, we still have 70% chance to lose all. There is not easy for us to earn from this situation.

```{r}
mean(number_of_plays)
```
**ADD**In this part, we calculate the average number of plays. From the chart above, we can obviously conclude we nearly never reach the max number of plays. So what's the average? it shows 344.1 far less than 1000. However, still more than W=300. That is caused by we need a longer road to take the money out.

Now we loose the limited of winning, which is we would quit when we earn 200 dollars. In this situation, we are greedy and want to earn more, but the reality is that we have lower chance (about 30%) to win out. For most time, we lose to 0. As for the mean earning, it is -78.60 this time. It also revealed. Under this strategy, the more greedy the easier it is to lose
...
number_of_plays[j] <- one_series(B = 200, W = 300, L = 200, M = 100) %>% length
}
```
...
**ADD**In this part, it generate a graph showing the walk-out-money. From the graph, most of the money is 0 or 200-300, which means, in most time we quit the game with 0 or win some, and the chance is about half half. Comparing the situation before, we have 51.4% chance to win all, but now we have more chance to win less. (not really big difference, 55.5% - 51.4%)

```{r}
mean(walk_out_money > 200)
```
**ADD**The estimated probability of walking out with extra cash for the parameters B = 200, W = 300, L = 200, M = 100 is 56.47%. This probability is close as before L = 1000, but a little bit higher, which meand when we limit L,we have more chance to win. And for the expected value, we lose less.

```{r}
mean(walk_out_money - 200)
```

**ADD**In this part, we calculate the expected winning and it is -35.33, which larger than L = 1000.Thus, it means when we gamble much, we lose much. No gambling is earning money from expected value!

```{r}
mean(number_of_plays)
```
**ADD**When we limit the number of plays to 200, it will drop to 156.7. That is we cut 80% of the upper limit but only reduce 25% average number of plays. 
...
number_of_plays[j] <- one_series(B = 200, W = 300, L = 1000, M = 300) %>% length
}
```
...
hist(walk_out_money, breaks = 100)
``` 
**ADD**In this part, it generate a graph showing the walk-out-money. From the graph, all of the money is 0 or 300, all looks similar to M=100. M=100 or M=300 don't affect too much because it's very hard to get wager =100 situation or we lose 7 times in a row. Thus, the graph does not change too much.

```{r}
mean(walk_out_money > 200)
```
**ADD**The estimated probability of walking out with extra cash for the parameters B = 200, W = 300, L = 1000, M = 300 is 52.67%. This probability is similar as before M = 100. It shows the Wager does not limit too much. We can speculate that when we reach the wager to 100, it is not a regular event. 
...
**ADD**In this part, we calculate the expected winning and it is -42.03. To my surprice, it is a little less than M=100. I think they should look similar, and this time is caused by random error.
...
**ADD**In this part, we calculate the average number of plays, which is 181.2. It is less than we put M = 100. That is because if we lose in a row, we may lose our money quickly because now we can lose 300 dollars for one time, but original version is just 100 dollars.
