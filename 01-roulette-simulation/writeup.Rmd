...
In this part, we calculate the expected winning and it is -44.4444, which means if I use the strategy, we all going to lose 44.44 as an expected value. **ADD** Althogh we have over 50% chance to win, we still have a expected value to lose money. That is we only have two choice: win 100 or lose 200. It is so easy to lose more money than win by the strategy.
...
hist(walk_out_money, breaks = 100)
```
**ADD**In this part, it generate a graph showing the walk-out-money. From the graph, all of the money is 0 or 400, which means, we cannot reach the max number of plays, we will quit the game either lose everything or reach our hoal, and the chance is about 70%-30%. Comparing the situation before, which is half half, this time we have less chance to quit while we win.


```{r}
mean(walk_out_money > 200)
```
**ADD**The estimated probability of walking out with extra cash for the parameters B = 200, W = 400, L = 1000, M = 100 is 29.67%. This probability is much lower than before which means we are less likely to be profitable in this scenario comparing with W=300.

```{r}
mean(walk_out_money - 200)
```
**ADD**In this part, we calculate the expected winning and it is -83.06, which means double as W = 300. Althogh we have 30% chance to win 200 dollars, we still have 70% chance to lose all. There is not easy for us to earn from this situation.

```{r}
length(one_series(B = 200, W = 400, L = 1000, M = 100))
```
**ADD**In this part, we calculate the average number of plays. From the chart above, we can obviously conclude we nearly never reach the max number of plays. So what's the average? it show 466 far less than 1000. However, still more than W=300. That is caused by we need a longer road to take the money out.
...
mean(walk_out_money > 200)
```
**ADD**The estimated probability of walking out with extra cash for the parameters B = 200, W = 300, L = 200, M = 100 is 51.33%. This probability is much similar as before L = 1000. It means the max number of plays does not affect the chance we win too much.

```{r}
mean(walk_out_money - 200)
```

**ADD**In this part, we calculate the expected winning and it is -45, which means similar as L = 1000.  Combine the output we have 51.3% to win, L does not affect the result too much, but we will win less if we win, because when L = 1000, we can win $100, but now maybe less than 100.

```{r}
length(one_series(B = 200, W = 300, L = 200, M = 100))
```
**ADD**When we limit the number of plays to 200, it will drop to 120. That is we cut 80% of the upper limit but only reduce 50% average number of plays. 
...
mean(walk_out_money > 200)
```
**ADD**The estimated probability of walking out with extra cash for the parameters B = 200, W = 300, L = 1000, M = 300 is 52.8%%. This probability is similar as before M = 100. It shows the Wager does not limit too much. We can speculate that when we reach the wager to 100, it is far from we lose all.
...
**ADD**In this part, we calculate the expected winning and it is -41.6. To my surprice, it is a little less than M=100. I think they should look similar, and this time is caused by random error.
...
**ADD**In this part, we calculate the average number of plays, which is 197. It is less than we put M = 100. That is because if we lose in a row, we may lose our money quickly because now we can lose 300 dollars for one time, but original version is just 100 dollars.
